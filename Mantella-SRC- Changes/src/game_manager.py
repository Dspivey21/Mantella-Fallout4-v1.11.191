...
class GameStateManager:
    ...
    def sentence_to_json(self, sentence_to_prepare: Sentence, topicID: int) -> dict[str, Any]:
        # Flatten actions from v0.14 dict format [{'identifier': 'x'}] to v0.13 string format ['x']
        # so Papyrus F4SE_HTTP.getStringArray() can parse them
        flat_actions = []
        for a in sentence_to_prepare.actions:
            if isinstance(a, dict) and 'identifier' in a:
                flat_actions.append(a['identifier'])
            elif isinstance(a, str):
                flat_actions.append(a)
            else:
                flat_actions.append(str(a))
        ...
        json_dict = {
            ...
            comm_consts.KEY_ACTOR_ACTIONS: flat_actions,
